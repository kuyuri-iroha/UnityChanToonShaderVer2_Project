...
////// Lighting:
                ...
                // RimLightの方向指定を独立させる
                float3 customRimLightDirection = normalize(mul( unity_ObjectToWorld, float4(((float3(1.0,0.0,0.0)*_Offset_X_Axis_RimLightBLD*10)+(float3(0.0,1.0,0.0)*_Offset_Y_Axis_RimLightBLD*10)+(float3(0.0,0.0,-1.0)*lerp(-1.0,1.0,_Inverse_Z_Axis_RimLightBLD))),0)).xyz);
                lightDirection = lerp(lightDirection, customRimLightDirection, _Is_RimLightBLD);
